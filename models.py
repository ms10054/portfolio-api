...
class Portfolio(db.Model):
    """Stores personal info, about, contact, and social links for a user."""
...
class Skill(db.Model):
    """A skill belonging to a user with a proficiency level."""
...
PROJECT_STATUSES = ["planned", "in_progress", "completed", "archived"]
...
class Project(db.Model):
    ...
    project_image = db.Column(db.String(255), nullable=True)
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    updated_at = db.Column(
        db.DateTime,
        default=lambda: datetime.now(timezone.utc),
